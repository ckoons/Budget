"""
Hermes Bridge for Budget MCP Tools

This module bridges Budget's FastMCP tools with Hermes MCP aggregator,
allowing Budget's tools to be discoverable and executable through Hermes.
"""
...
class BudgetMCPBridge(MCPService):
    """
    Bridge between Budget's FastMCP tools and Hermes MCP aggregator.
    
    This class allows Budget to register its tools with Hermes while
    maintaining the existing FastMCP implementation.
    """
    
    def __init__(self, budget_engine, component_name: str = "budget"):
        """Initialize the Budget MCP Bridge."""
    ...
    async def initialize(self):
        ...
        # Load FastMCP tools
        try:
            from budget.core.mcp import get_all_tools
            self._fastmcp_tools = get_all_tools(self.budget_engine)
    ...
    async def register_default_tools(self):
        ...
        info_tool = ComponentInfoTool(
            ...
            component_description="Budget Management System for token allocation and cost optimization"
        )
    ...
    async def register_fastmcp_tool(self, fastmcp_tool: Dict[str, Any]):
        ...
        async def handler(parameters: Dict[str, Any]) -> Dict[str, Any]:
            ...
            from budget.api.mcp_endpoints import process_request_func
            ...
            response = await process_request_func(self.budget_engine, request)
    ...
    async def _get_health_status(self) -> Dict[str, Any]:
        """Get health status from budget engine."""
        try:
            if self.budget_engine:
                # Get engine status
                allocations = len(self.budget_engine.allocations)
                active_allocations = sum(1 for a in self.budget_engine.allocations.values() if a.is_active)
                ...
                return {
                    ...
                    "details": {
                        ...
                        "tracking_active": hasattr(self.budget_engine, 'tracking') and self.budget_engine.tracking is not None
                    }
                }
            else:
                return {
                    "status": "starting",
                    "details": {"message": "Budget engine initializing"}
                }
        except Exception as e:
            return {
                "status": "unhealthy", 
